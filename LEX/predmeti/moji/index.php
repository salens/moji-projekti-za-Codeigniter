...
if(isset($_SESSION['id']))
{
  ...
  $radnikId = $_SESSION['id'];
  $predmet = new Predmet;
  $predmetMetode = new predmetMetode;
  $predmeti = $predmetMetode->citaSvePredmete(NULL,$radnikId);
  ...
  <p><strong>Spisak predmeta koje zadužuje: " . $_SESSION["ime"] .' '.$_SESSION["prezime"] . " </strong></p>
  <table>
  ...
  foreach ($predmeti as $predmet)
  {
    ...
    $br +=1;
    $predmetiSadrzaj .= '<tr><td>'.$br.'.</td><td>'. $predmet->ime .'</td><td>'. $predmet->prezime .'</td><td>'. $predmet->brojPredmeta .'</td><td>'. $predmet->sudskiBroj .'</td><td>'. $predmet->datumVreme .'</td><td>'. $predmet->status .'</td><td>'. $predmet->zaduzenIme .' ' . $predmet->zaduzenPrezime.'</td><td><input name"hidId'. $predmet->id .'" type="hidden" value="'. $predmet->id .'"/></td><td><a href="../detalji/?id='.$predmet->id.'" class="">Detalji</a></td><td><a href="./obaveze/" class="">Obaveze</a></td></tr>';
  }
